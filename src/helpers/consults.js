...
export const findMadeTransfers = async (idAccount = "") => {
  const query = await Transfer.findAll({
    where: { idAccountOrigin: idAccount },
    include: [{ model: Account, as: "AccountDestiny" }],
  });

  const transfers = query.map((transfer) => {
    const { dateReport, amountOrigin, amountDestiny, taxes, description, AccountDestiny } =
      transfer.dataValues;
    ...
    return {
      date,
      amount,
      amountDestiny: amountDestinyFixed,
      exchange: pTaxes,
      description,
      bankName,
      numberAccount,
